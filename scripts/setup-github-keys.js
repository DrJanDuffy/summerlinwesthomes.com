...
function checkGpgKeyExists(email) {
  const result = runCommand(`gpg --list-secret-keys "${email}"`, {
    silent: true,
  });
...
async function main() {
  ...
  const name = await getUserInput(
    ...
    gitName.success ? gitName.output.trim() : ''
  );
  const email = await getUserInput(
    'Enter your email address',
    gitEmail.success ? gitEmail.email.trim() : ''
  );
  ...
  if (checkSshKeyExists(sshKeyName)) {
    ...
    const overwrite = await getUserInput(
  ...
  if (addResult.success) {
    ...
    printWarning(
      'Could not add SSH key to agent. This is normal on some systems.'
    );
  }
  ...
  console.log('');
  printWarning(
    'Copy the above SSH public key and add it to your GitHub account at:'
  );
...
%commit`;
  ...
  console.log('================================================');
  const gpgExport = runCommand(`gpg --armor --export ${gpgKeyId}`, {
    silent: true,
  });
  ...
  if (
    sshTest.success &&
    sshTest.output.includes('successfully authenticated')
  ) {
    printSuccess('SSH connection to GitHub successful!');
  } else {
    printWarning(
      "SSH connection test failed. This is normal if you haven't added the key to GitHub yet."
    );
    printWarning(
      'Add the SSH key to GitHub and run this script again to test the connection.'
    );
  }
